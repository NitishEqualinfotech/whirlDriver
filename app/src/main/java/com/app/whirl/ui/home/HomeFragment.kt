...
import com.app.whirl.activities.profile.ProfilePhotoActivity
...
import kotlinx.android.synthetic.main.fragment_home.rlLoader
import kotlinx.android.synthetic.main.fragment_home.tvDropAddress
import kotlinx.android.synthetic.main.fragment_home.tvName
import kotlinx.android.synthetic.main.fragment_home.tvPickAddress
import kotlinx.android.synthetic.main.otp_verify_popup.*
import kotlinx.android.synthetic.main.start_ride_popup.*
import kotlinx.android.synthetic.main.vrifyotp_fragmentt.*
...
class HomeFragment : Fragment(), InterfaceName {
    ...
    private val mMessageReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.extras!!.getString("cond").equals("1")) {
                updateDriverLocation()
            }
    ...
    fun on() {
        ...
        moon_onnline.visibility = View.VISIBLE
    ...
    fun off() {
        ...
        moon_onnline.visibility = View.GONE
        rlStartRidevry.visibility = View.GONE
   ...
   */
        ...
        btnOtpmatch.setOnClickListener {
            OtpVerify()
        }



        tvCompletetrip.setOnClickListener {
        ...
        tvVerifyOtp.setOnClickListener {
            var otp =
                etCode1.text.toString() + "" + etCode2.text.toString() + "" + etCode3.text.toString() + "" + etCode4.text.toString()
                ...
                if (response.isSuccessful) {
                    ...
                    if (response.body()!!.success.equals("true")) {
                        ...
                        if (!response.body()!!.data[0].time.isNullOrEmpty()) {
                            tvHr.setText(response.body()!!.data[0].time + "Hrs")
                        }
                        ...
                        if (!response.body()!!.data[0].profile_photo.isNullOrEmpty()) {
                            Glide.with(activity).load(response.body()!!.data[0].profile_photo)
                ...
                if (response.isSuccessful) {
                    ...
                    if (response.body()!!.success) {
                        if (response.body()!!.data.size > 0) {
                            ...
                            if (!response.body()!!.data[0].distance.isNullOrEmpty()) {
                                tvBookingDistance.setText(response.body()!!.data[0].distance + " miles")
                            }
                            ...
                            if (!response.body()!!.data[0].amount.isNullOrEmpty()) {
                                tripPrice = response.body()!!.data[0].amount
                                tvPricevryy.setText("$ " + response.body()!!.data[0].amount)
                            }

                            if (!response.body()!!.data[0].user_name.isNullOrEmpty()) {
                                custName = response.body()!!.data[0].user_name
                                tvNamevryy.setText(response.body()!!.data[0].user_name)
                            }

                            if (!response.body()!!.data[0].distance.isNullOrEmpty()) {
                                tvEarnedvryy.setText(response.body()!!.data[0].distance + " miles")
                            }

                            if (!response.body()!!.data[0].pickup_address.isNullOrEmpty()) {
                                pickupAddress = response.body()!!.data[0].pickup_address
                                tv_pickuplocation.setText(response.body()!!.data[0].pickup_address)
                            }
                            ...
                            if (!response.body()!!.data[0].amount.isNullOrEmpty()) {
                                tripPrice = response.body()!!.data[0].amount
                                tvPricevry.setText("$ " + response.body()!!.data[0].amount)
                            }

                            if (!response.body()!!.data[0].user_name.isNullOrEmpty()) {
                                custName = response.body()!!.data[0].user_name
                                tvNamevry.setText(response.body()!!.data[0].user_name)
                            }

                            if (!response.body()!!.data[0].distance.isNullOrEmpty()) {
                                tvEarnedvry.setText(response.body()!!.data[0].distance + " miles")
                            }

                            if (!response.body()!!.data[0].drop_address.isNullOrEmpty()) {
                                dropAddress = response.body()!!.data[0].drop_address
                                drpe_location.setText(response.body()!!.data[0].drop_address)
                            }
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    if (response.body()!!.success) {
                        card_online.visibility = View.GONE
                        button_offline.visibility = View.GONE
                        rlStartRidevry.visibility = View.VISIBLE
                        if (!pickup_latitude.isNullOrEmpty()) {
                            //val gmmIntentUri = Uri.parse("google.navigation:q="+pickup_latitude+","+pickup_long + "&mode=d")
                            val gmmIntentUri =
                                Uri.parse("google.navigation:q=28.6921164,76.8111449&mode=d")
                            val mapIntent = Intent(Intent.ACTION_VIEW, gmmIntentUri)
                            mapIntent.setPackage("com.google.android.apps.maps")
                            startActivity(mapIntent)
    ...
    private fun OtpVerify() {
        ...
        var hashMap: HashMap<String, String> = HashMap<String, String>()
        hashMap.put("booking_id", booking_id)
        hashMap.put("otp", number_edit_text.text.toString())

        val apiCall = ApiInterface.create().BookingOtp(hashMap)
        apiCall.enqueue(object : Callback<OtpMatchResponse> {
            override fun onResponse(
    ...
    private fun SendNotification() {
        ...
        var hashMap: HashMap<String, String> = HashMap<String, String>()
        hashMap.put("booking_id", booking_id)
        hashMap.put("user_id", SharedPreferenceUtils.getInstance(requireContext())?.getStringValue(ConstantUtils.USER_ID,"").toString())
       // hashMap.put("otp", number_edit_text.text.toString())

        val apiCall = ApiInterface.create().sendnotificaiotn(hashMap)
...
/*    override fun getride(name: String) {
        if (name.equals("1")){
            updateDriverLocation()
        }
